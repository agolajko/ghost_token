...
@pytest.mark.asyncio
async def test_get_hash(branch_contract: StarknetContract):
    # get_state_update on the block the contract was delpoyed into. Returns JSON
    # from the JSON find the contract_hash corresponding to the account_address

    # sequence:
    # get transaction hash
    # feed transaction hash to get_transaction, returns block_hash and block_number
    # use block_hash and block_number to get block_state_updates
    # from block_state_updates use the contract address to find the contract_hash

    # breakpoint()

    # get the  deployed contract address
    contract_addr = branch_contract.contract_address
    print("contract_addr")
    print(contract_addr)
    print("branch_contract.transaction_hash")
    print(branch_contract.deploy_execution_info)

    # get the block_hash and block_number from the transaction hash
...
# get the contract hash for the above deployed contract
# needed for verify_both_branches
...
#     root_hash, merkle_branch_high, merkle_branch_low = generate_proof("/mnt/volume_lon1_01/pathfinder/goerli.sqlite", 24_444, "0x35572dec96ab362c35139675abc4f1c9d6b15ee29c98fbf3f0390a0f8500afa", "balances", 1042400286771102661652363919924244740833084544629561888149967508378012757441)
...
#     leaf_high = merkle_branch_high[0]
#     # real_branch_rest = real_branch_int[1:]
#     # total_len = 251
#     # root_hash = 2100660188095084288620515079086904656261336581214533446535447348142332014717

#     res_hash = await branch_contract.verify_both_branches(leaf, merkle_branch_low, total_len, root_hash).call()
#     return()
